...
def arquivo_mes_ano(caminho: Path) -> ArquivoMensal | None:
    nome = caminho.stem
    match = re.search(
        r"\b(JANEIRO|FEVEREIRO|MAR[CÇ]O|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO)\s+(\d{4})\b",
        nome,
        flags=re.IGNORECASE,
    )
    if not match:
...
def extrair_registros_excel(caminho: Path) -> pd.DataFrame:
    ...
    for col in ["Data", "Nome", "Modelo", "Pedido", "Vendedor", "AGR", "Origem", "Parceiro", "CPF/CNPJ"]:
        if col not in df.columns:
            df[col] = ""
    return df[
        ["Data", "Nome", "CPF/CNPJ", "Parceiro", "Modelo", "Pedido", "Valor Planilha", "Vendedor", "AGR", "Origem"]
    ].copy()
